"""Hyperparameter-tune XGBoost per (dataset, horizon) and re-evaluate on test.

The original main_results.csv used XGBoost defaults (max_depth=3,
n_estimators=300, lr=0.05). After lifting NeuralForecast models to a
full training budget, the symmetric move for XGBoost is a small grid
search on the validation split (no test-set leakage), then a final
multi-seed evaluation on the test split with the best configuration.

Outputs are appended to results/main_results_v2.csv and per-window
errors / prediction samples land in results/window_errors and
results/prediction_samples respectively, mirroring the NF rerun.

Best hyperparameters per (dataset, horizon) are written to
results/xgboost_tuning.csv together with validation MAE for every grid
point so the choice can be inspected later.
"""
...
TUNING_LOG = ROOT / "results" / "xgboost_tuning.csv"
...
def fit_predict_xgb(
    *,
    ...
    # Single-window latency estimate (CPU): one warm-up + 5 timed calls.
...
def append_tuning_log(rows: Iterable[dict]) -> None:
    ...
    exists = TUNING_LOG.exists()
    with TUNING_LOG.open("a", newline="") as fh:
...
def tune_one(name: str, horizon: int, input_length: int, log_path: Path) -> dict:
    splits = make_splits(name, horizon, input_length)
    target = DATASETS[name][1]
    scaler = splits["scaler"]
    print(f"[tune] {name}/H{horizon}: train_windows={len(splits['x_train'])}, val_windows={len(splits['x_val'])}", flush=True)
    ...
    for params in GRID:
        ...
        msg = (f"[tune] {name}/H{horizon} {params} val_MAE={val_mae:.2f} fit_time={fit_time:.1f}s")
    ...
    append_tuning_log(grid_results)
    print(f"[tune] {name}/H{horizon} BEST: max_depth={best['max_depth']} n_estimators={best['n_estimators']} "
          f"lr={best['learning_rate']} val_MAE={best['val_MAE']:.2f}", flush=True)
...
def evaluate_seeds(
    ...
    for seed in SEEDS:
        y_pred_scaled, model, fit_time, inf_ms, n_params = fit_predict_xgb(
        ...
        msg = (f"[eval] {name}/H{horizon} seed{seed} MAE={metrics['MAE']:.2f} "
               f"sMAPE={metrics['sMAPE']:.2f} fit_time={fit_time:.1f}s")
        print(msg, flush=True)
        with log_path.open("a") as fh:
            fh.write(msg + "\n")
...
def main() -> None:
    ...
    parser.add_argument("--log", type=Path, default=ROOT / "results" / "xgboost_tune_log.txt")
    ...
    for name in args.datasets:
        for horizon in args.horizons:
            ...
            try:
                tune_out = tune_one(name, horizon, args.input_length, args.log)
            except Exception as exc:
                msg = f"[ERROR tune] {name}/H{horizon}: {exc}"
                print(msg, flush=True)
            ...
            try:
                ...
                msg = f"[ERROR eval] {name}/H{horizon}: {exc}"
                print(msg, flush=True)
                with args.log.open("a") as fh:
                    fh.write(msg + "\n")
                continue
            ...
            with args.log.open("a") as fh:
                fh.write(f"[done] {name}/H{horizon} in {elapsed/60:.1f} min\n")

    total = time.perf_counter() - overall_start
    print(f"All XGBoost tuning + eval done in {total/60:.1f} min", flush=True)
    with args.log.open("a") as fh:
        fh.write(f"DONE total {total/60:.1f} min\n")
